function [t,y,yp] = rk3 %PRO VELKE LAMBDA EXPLICITNI METODA HAVARUJE, NEBOT musi platit podminka: tau < 2/lambda...
format long
...
yp = 3000*(1-exp(-0.002*t)); %presne reseni
[tMatLab,yMatLab] = ode45(f, cas, y0);

y', yp' %vypis vysledku
maxChyba=max(abs(yp-y)) %chyba v reseni

%zachrana zivota v nadrzi: y(t) = 1; t=? -> t=0.1667hod = 10min (urceno z presneho reseni)
